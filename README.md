...
- Shell functions defined in `~/.zshrc`: `seminary-new`, `seminary-build`, `seminary-new-review`, `seminary-build-review`, `seminary-import`, `seminary-fix-refs`, `sem-wc`, `sem-cover-wc`, `seminary-quiz`, `seminary-quiz-from-pdf` — the `seminary` menu script sources `~/.zshrc` to call these directly.
